...
EosEncoderBlock::EosEncoderBlock(MainController* controller, QString uid)
    ...
    , m_feedback(this, "feedback", true, true)
{
    connect(m_controller->midi(), SIGNAL(messageReceived(MidiEvent)), this, SLOT(onMidiMessage(MidiEvent)));
    connect(&m_feedback, SIGNAL(valueChanged()), this, SLOT(onFeedbackChanged()));

    emit(m_feedback.valueChanged());
}
...
void EosEncoderBlock::onFeedbackChanged() {
    if (m_feedback)
